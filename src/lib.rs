...
//! ```
use std::{
    error::Error, fmt::Display, num::ParseFloatError,
    ops::RangeInclusive, str::FromStr,
};

use egui::{
    vec2, Rect, Response, RichText, Sense, TextStyle, Ui, Vec2,
    WidgetText,
};
...
impl Display for ParseKeyActionError {
    fn fmt(
        ...
        write!(
            ...
            "{} while parsing \"{}\"",
            match self.kind {
                ParseKeyActionErrorKind::FromText =>
                    "Expected FromText",
                ParseKeyActionErrorKind::Layout => "Expected Layout",
                ParseKeyActionErrorKind::Key => "Expected Key",
                ParseKeyActionErrorKind::Modifier =>
                    "Expected Modifier",
                ParseKeyActionErrorKind::Unknown =>
                    "Unexpected Token",
            },
            self.parsing,
        )
...
impl FromStr for Button {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        while let Some(colpos) = inner[search_from..].find(":") {
            if colpos > 0
                && inner.get(colpos - 1..colpos) == Some("\\")
            {
...
impl FromStr for Layout {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let Some((l, r)) = s.split_once("\n") else {
            error.cause = "Must have at least 2 lines: name and row"
                .to_string();
            return Err(error);
        };
        ...
        if l.contains(DO_NOT_USE_CHARS) {
            error.cause =
                "Layout name cannot contain (){}[]\":;".to_string();
            return Err(error);
        };
        ...
        let rows = r
            ...
            .map(Row::from_str)
            .filter_map(|res| {
                res.map_err(|e| error.inner.push(e)).ok()
            })
            .collect();
...
fn parse_key_action() {
    ...
    assert_eq!(
        Ok(KeyAction::Layout("QWERTY_Mobile".to_string())),
        action
    );
    let action = KeyAction::from_str("Layout(QWERTY_Mobile))");
...
impl ModState {
    fn mod_str(&self, modifier: Modifier) -> String {
        match self {
            ...
            Self::Once => format!(
                "_MOD_{}_ONCE",
                modifier.to_string().to_uppercase()
            ),
            Self::Hold => format!(
                "_MOD_{}_HOLD",
                modifier.to_string().to_uppercase()
...
impl VirtualKeyboard {
    ...
    pub fn with_button_width(
        mut self,
        width: impl Into<RangeInclusive<f32>>,
    ) -> Self {
    ...
    fn buttons(&mut self, ui: &mut Ui) -> Response {
        let Some(layout) = self.layouts.get(self.current).cloned()
        else {
            return ui.response();
        };
        ...
        let btn_w = ((h_space
            - spacing.item_spacing.x * (btn_mul - 1.0))
            / btn_mul)
            .clamp(*btn_w_r.start(), *btn_w_r.end());
        let btn_h =
            self.button_h.unwrap_or(spacing.interact_size.x * 0.9);
        ...
        let base_resp =
            ui.interact(kbd_rect, base_id, Sense::hover());
        ...
        let draw_btn =
            |button: &Button, offset: &mut Vec2, ui: &mut Ui| {
                let kbd_min = kbd_rect.min.to_vec2();
                Self::draw_one(
                    kbd_min, btn_std, itm_spc, button, offset, ui,
                )
            };

        let actions = layout.rows.iter().enumerate().flat_map(
            |(row_idx, row)| {
                let btn_mul = row.takes_width();
                let mut offset = vec2(
                    (kbd_rect.width()
                        - (btn_std + itm_spc).x * btn_mul
                        + itm_spc.x)
                        * 0.5,
        ...
        let resp = actions.fold(
            base_resp,
            |acc, (action, text, response)| {
    ...
    fn draw_one(
        ...
        let size = vec2(
            btn_std.x
                + (btn_std.x + itm_spc.x) * (button.width_mult - 1.0),
            btn_std.y,
        );
        ...
        let text = WidgetText::RichText(
            RichText::new(&button.text).monospace(),
        )
    ...
    pub fn layout_name(&self) -> String {
        ...
        let mid =
            layout.name.find("_MOD").unwrap_or(layout.name.len());
